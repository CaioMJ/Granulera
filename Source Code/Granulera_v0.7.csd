...
rslider bounds(576, 160, 100, 70) range(0.001, 1, 1, 0.35, 0.001) text("Frequency") channel("FilterFreq") trackercolour(188, 151, 49, 255) textcolour(255, 255, 255, 255)
rslider bounds(678, 160, 100, 69) range(0.001, 1, 0.001, 0.35, 0.001) text("Start/End Freq") channel("FilterRange") trackercolour(188, 151, 49, 255) textcolour(255, 255, 255, 255)
rslider bounds(778, 160, 100, 69) range(0.001, 1, 1, 1, 0.001) text("Peak Freq") channel("FilterPeak") trackercolour(188, 151, 49, 255) textcolour(255, 255, 255, 255)

rslider bounds(870, 160, 100, 70) range(1, 1000, 1, 0.2, 0.01) text("Resonance") channel("FilterReson") trackercolour(188, 151, 49, 255) textcolour(255, 255, 255, 255)
...
hslider bounds(982, 162, 200, 36) range(0, 1, 0, 1, 0.01) channel("DelaySend") text("Send") trackercolour(188, 151, 49, 255) textcolour(255, 255, 255, 255)
...
hslider bounds(978, 504, 200, 36) range(0, 10, 0.1, 0.5, 0.01) text("Attack") channel("AmpAttack") trackercolour(188, 151, 49, 255) textcolour(255, 255, 255, 255)
...
0dbfs = 1
//TO DO:
;MAKE kFilterFreq INVERSELY PROPORTIONAL TO kFilterPeak
;FIX OSCILLATOR 3
...
//MIDI:
    iFreqMIDI cpsmidi
    gaAmpEnv mxadsr iAttack, iDecay, iSustain, iRelease
...
//PORTK:
    ...
    gkGlobalVol portk gkGlobalVol, 0.1
...
//FILTERING: 
    ...
    if iFilterAttack > 0.01 then
        ...
        kFilterEnv1  expsegr 1, iFilterDecay, 1 * iFilterSustain * iFilterFreq, iFilterRelease, iFilterFreq
    endif
    ...
    //Frequency
    kFilterFreqTotal = aFilterEnv + kFreqTotal ;(kFilterFreq + aFilterEnv) + kFreqTotal
...
//PANNING
    gaSigL, gaSigR pan2 aSigFilter * gkGlobalVol, kGlobalPan

//OUTPUT
    outs gaSigL * gaAmpEnv, gaSigR * gaAmpEnv
    
//EFFECT SENDS
    gkReverbSend chnget "ReverbSend"
    gaReverbSendL = gaSigL * gaAmpEnv * gkReverbSend
    gaReverbSendR = gaSigR * gaAmpEnv * gkReverbSend
    
    gkDelaySend chnget "DelaySend"
    gaDelaySendL = gaSigL * gaAmpEnv * gkDelaySend
    gaDelaySendR = gaSigR * gaAmpEnv * gkDelaySend
endin
...
if gkDelaySend > 0 then
    ...
    aTapL       deltapi kDelayTimeLeft
                delayw gaDelaySendL + (aTapL * kDelayFeedback)
    aBufferOutR delayr 10
    aTapR       deltapi kDelayTimeRight
                delayw gaDelaySendR + (aTapR * kDelayFeedback)
